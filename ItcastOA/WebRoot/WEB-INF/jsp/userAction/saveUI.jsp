...
<head>
    ...
    <script type="text/javascript">
    	function checkUser(o){
    		var actionStr = $("form[0]").attr("action");
    		//如果是add方法,需要做服务器端用户是否存在判断
    		if(actionStr.indexOf("add")>=0){
    			var loginName = $("#loginName").val();
    			$.post("userAction_checkUser.action",{"loginName":loginName},function(data){
    				alert(data);<%----%>
    				
    				if(data=="1"){
    					$("#check").html("<font color='red'>登录名不能为空</font>");
    				}else if(data=="2"){
    					$("#check").html("<font color='red'>登录名已存在</font>");
    				}else{
        ...
        <div class="ItemBlockBorder">
            <div class="ItemBlock">
                <table cellpadding="0" cellspacing="0" class="mainForm">
                    ...
                    <tr><td>登录名</td>
                        <td><s:textfield id="loginName" name="loginName" cssClass="InputStyle required" onblur="checkUser(this);"/> *
							（登录名要唯一）
							<div id="check"></div>
						</td>
